...
// ----------------------------  FINISH FUNCTION INPUT DATA DOKTOR -------------------------- //
	...
	public function cetak() {
		...
		if(!empty($cek)){
			...
			$NoUjian = $this->app_model->CariNoUjian($nik);
			$d['nik'] = $this->session->userdata('nik');
			$d['nama'] = $this->session->userdata('nama');
			$d['NoUjian'] = $NoUjian;
			// print_r($d['NoUjian']);exit();
			...
			if($num_rows > 0) {
			  ...
			  // judul report
			  $pdf->setNama("DATA TRANSAKSI UNTUK BARANG ".$nik);
			  // buat halaman
			  $pdf->AliasNbPages();
			  // Potrait ukuran A4
			  $pdf->AddPage("P","A4");
				foreach($data->result() as $t){
					...
					$pdf->SetX(6);
					$pdf->Cell(96,3,'Universitas Nahdlatul Ulama Indonesia (UNU)',0,1,'C');
					$pdf->SetX(6);
					$pdf->Cell(96,3,'Jl. Taman Amir Hamzah No. 5, Jakarta Pusat 10430',0,1,'C');
					$pdf->SetX(6);
					...
					$pdf->Cell(5,5,':',0,0,'C',false);
					$pdf->Cell(61,5,$t->lokasi,0,1,'L',false);
					...
					*/
					...
					$pdf->Cell(17,3,'30 Juli 2019','LR',0,'L');
					...
					$pdf->Cell(17,3,'31 Juli 2019','LBR',0,'L');
					...
					$pdf->Cell(17,3,'01 Agst 2019','LBR',0,'L');
					...
					$pdf->SetX(6);
					$pdf->Cell(198,4,'Universitas Nahdlatul Ulama Indonesia',0,1,'C');
					$pdf->SetX(6);
					...
					$pdf->Cell(35,5,'Asal Sekolah/Universitas',0,0,'L',false);
					$pdf->Cell(5,5,':',0,0,'C',false);
			  ...
			  //$a->Image($file_foto,160,173,$new_w,$new_h);
				  // $pdf->Output('KARTU_UJIAN_'.$nik.'.pdf','D');
				  $pdf->Output('KARTU_UJIAN_'.$nama.'_'.$nik.'.pdf','D');
			}else{
