...
class Laporan extends CI_Controller {
	...
	public function lap_pmb() {
		...
		if(!empty($cek)){
			...
			$d['jurusan'] = $this->app_model->manualQuery("SELECT * FROM tbjurusan WHERE tampil='Ya'");
	...
	public function cetak_lap_pmb()
	{
		...
		if(!empty($cek)){
			...
			$tanggal = $this->app_model->tgl_sql($tgl);
			if($jurusan=='all'){
				if($tanggal==""){
					$d['data'] = $this->app_model->manualQuery("SELECT * FROM mspcmb WHERE substr(ThAjaran,1,4)='$th' AND NoUjian<>'' ORDER BY NoUjian");
				}elseif($tanggal <> ''){
					$d['data'] = $this->app_model->manualQuery("SELECT * FROM mspcmb WHERE substr(ThAjaran,1,4)='$th' AND NoUjian<>'' AND TglDaftar = '$tanggal' ORDER BY NoUjian");
				}
				
			}else{
				if($tanggal == ""){
					$d['data'] = $this->app_model->manualQuery("SELECT * FROM mspcmb WHERE substr(ThAjaran,1,4)='$th' AND prodi='$jurusan' AND NoUjian<>'' ORDER BY NoUjian");
				}else{
					$d['data'] = $this->app_model->manualQuery("SELECT * FROM mspcmb WHERE substr(ThAjaran,1,4)='$th' AND prodi='$jurusan' AND NoUjian<>'' AND TglDaftar = '$tanggal' ORDER BY NoUjian");
				}
			}
	...
	public function cetak_lap_pmb_ruangan()
	{
		...
		if(!empty($cek)){
			...
			$d['ruang'] = $ruang;
			$d['data'] = $this->app_model->manualQuery("SELECT * FROM mspcmb WHERE ThAjaran='$th' AND RUjian='$ruang' and slip_pembayaran !='' ORDER BY NoUjian");
	...
	public function nomor_meja()
	{
		...
		if(!empty($cek)){
			...
			$d['ruang'] = $this->app_model->manualQuery("SELECT RUjian FROM mspcmb WHERE RUjian<>'' and slip_pembayaran !='' GROUP BY RUjian");
