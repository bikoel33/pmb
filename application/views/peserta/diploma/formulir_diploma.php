...
$(document).ready(function() {
   ...
   $('#sel_prodi2').change(function(){
	...
	$("#simpan").click(function(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/peserta/home/simpan_biodata_diploma",  // load controller
			data	: string,
			cache	: false,
			dataType  : 'json',
			success	: function(data){
				if(data.status != 'error'){
			   		window.location.assign("<?php echo site_url();?>/peserta/home/foto_diploma")  // load controller
            	}
            	alert(data.msg);
			}
		});
