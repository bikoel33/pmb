...
$(document).ready(function() {
	...
	$("#simpan").click(function(){
		...
		if(NmAsalSek.length==0){
			$('.bottom-right').notify({
				message: {text: 'Maaf, Universitas tidak boleh kosong'},type:'info'
			}).show();
			$("#AsalSek").focus();
			return false();
		}
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/peserta/home/simpan_biodata_magister",
			data	: string,
			cache	: false,
			dataType  : 'json',
			success	: function(data){
				if(data.status != 'error'){
			   		window.location.assign("<?php echo site_url();?>/peserta/home/foto_magister")
            	}
            	alert(data.msg);
			}
		});
...
<tr>
	<td>Tempat Lahir</td>
	<td><input type="text" name="TmptLhr" id="TmptLhr" class="span5" autofocus></td>
</tr>
<tr>
	<td>Tanggal Lahir</td>
...
<!-- <tr>	
	...
	<td>
		...
		<option value="">-PILIH-</option>
		<//?php foreach ($l_sekolah->result() as $t) { ?>
