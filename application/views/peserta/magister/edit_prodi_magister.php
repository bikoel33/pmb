	...
	$("#simpan").click(function(){
		...
		*/
		if (confirm('Pilihan Jurusan tidak dapat diulang, apakah yakin akan menyimpannya ?')){
			...
			$.ajax({
				type	: 'POST',
				url		: "<//?php echo site_url(); ?>/peserta/home/simpan_edit_prodi_magister",
				data	: string,
				cache	: false,
				dataType   : 'json',
				success	: function(data){
					if(data.status != 'error'){
						window.location.assign("<//?php echo site_url();?>/peserta/home/edit_survey_magister");
					}
					alert(data.msg);
				}
			});
...
<form id="my-form" name="my-form" method="POST" action="#">
	<table class="table table-bordered table-striped table-hover">
		<tbody>
			<tr>
				<td colspan="2"><h4>H. Pilihlah Program Studi dibawah ini</h4>
				<input type="hidden" name="NoUjian" id="NoUjian" class="span2" value="<?php echo $this->app_model->CariNik($this->session->userdata('nik'));?>">
				<input type="hidden" name="RUjian" id="RUjian" class="span2" value="<?php echo $this->app_model->CariRuangUjian($this->session->userdata('nik'));?>">
			...
			<tr>
				<td colspan="2">
					<center>
						<a href="<?php echo base_url();?>index.php/peserta/home/edit_surat_pernyataan_magister" class="btn btn-info"><i class="icon-hand-left icon-white"></i> Kembali</a>
						...
						<a href="<?php echo base_url();?>index.php/peserta/home/edit_survey_magister" class="btn btn-danger"><i class="icon-thumbs-up icon-white"></i> Abaikan</a>
					</center>
				</td>
			</tr>
		</tbody>
