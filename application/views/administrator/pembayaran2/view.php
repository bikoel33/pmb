...
function Cari_Bukti(){
	...
	$("#images").attr('src','<?php echo base_url();?>peserta/pembayaran2/'+file);
}
...
function cari_data(){
	...
	$.ajax({
		type	: "POST",
		url		: "<?php echo site_url('administrator/pembayaran2/cari_data'); ?>",
	...
	if (row){  
		$.messager.confirm('Confirm','Apakah Pembayaran Valid???',function(r){  
			if (r){  
				$.ajax({
					type	: "POST",
					url		: "<?php echo site_url('administrator/pembayaran2/update_valid'); ?>",
                    data	: 'nik='+nik,
					success	: function(data){
						$.messager.show({
							title:'Info',
	...
	if (row){  
		$.messager.confirm('Confirm','Apakah Pelunasan Akan Dihapuskan???',function(r){  
			if (r){  
				$.ajax({
					type	: "POST",
					url		: "<?php echo site_url('administrator/pembayaran2/update_tidak_valid'); ?>",
                    data	: 'nik='+nik,
					success	: function(data){
						$.messager.show({
							title:'Info',
...
<table id="datagrid-crud" title="Daftar <?php echo $judul;?>" class="easyui-datagrid" style="width:auto;height:auto;" url="<?php echo site_url('administrator/pembayaran2/view'); ?>?grid=true" toolbar="#toolbar" pagination="true" rownumbers="true" fitColumns="true" singleSelect="true" collapsible="true">
	<thead>
		<tr>
